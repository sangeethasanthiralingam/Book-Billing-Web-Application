...
class BillBuilderTest {
    ...
    void testCreateEmptyBill() {
        ...
        // When
        Bill bill = BillBuilder.createNewBill().withCustomer(testCustomer).withCashier(testCashier).build();
        // Then
        assertNotNull(bill);
    ...
    void testBuildBillWithUsers() {
        // When
        Bill bill = BillBuilder.createNewBill().withCustomer(testCustomer).withCashier(testCashier)
                .build();
    ...
    void testAddItemsAndCalculateTotals() {
        ...
        // When
        Bill bill = BillBuilder.createNewBill().withCustomer(testCustomer).withCashier(testCashier).addItem(item1).addItem(item2).build();
    ...
    void testBuildCompleteBill() {
        ...
        // When
        Bill bill = BillBuilder.createNewBill().withCustomer(testCustomer).withCashier(testCashier)
                .addItem(item).withPaymentMethod("CASH").build();
